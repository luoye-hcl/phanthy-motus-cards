...
| 项目 | 值 |
|---|---|
| 命令 Topic | `/hand_controller/commands` |
...
{"ok": true, "state": "idle",
 "safety": {"ros_publisher_available": true,
   ...
   "other_publishers": [{"node_name": "simple_actions_server"}]}}
```

### 备注

门禁检查通过:XHand Lite 12 关节,lifecycle=active,joint_state fresh,q5_fsm=READY。与 hand_gesture 共享 hand 发布器。

## 完整验收清单

- [x] MCP tools/list 确认工具 `hand_control` 已注册
