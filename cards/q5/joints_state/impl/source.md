# 实现来源(source)—— joints_state

- **源仓库**:`robotera-q5-driver`(image: `robotera-q5`)
- **文件路径**:`joints_state.py`
- **类名**:`Plugin`
...
> `impl/joints_state.py` 为**摘录**(`Plugin` 本体),便于审核理解返回值如何产生;不保证脱离真实驱动独立运行。以源仓库为准。
> 本目录不含任何口令/密钥。
...
驱动(Domain 211 CycloneDDS)订阅 `/joint_states` → `build(snapshot)` 生成 JSON → 发布到 Agent Core 桥接 topic `/{ns}/q5/joints_state`(Domain 42 FastDDS)。
